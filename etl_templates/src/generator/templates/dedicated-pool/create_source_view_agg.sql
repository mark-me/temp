...
{% if 'Expression' in attributemapping %}[{{attributemapping.AttributeTarget.Code}}]  = {{sqlexpression.get(attributemapping.Expression)}}({{attributemapping.AttributesSource.EntityAlias}}.[{{attributemapping.AttributesSource.Name}}])
{% elif CodeModel|upper == 'DA_CENTRAL' and Expression not in attributemapping %}[{{attributemapping.AttributeTarget.Code}}]  = {{attributemapping.AttributesSource.EntityAlias}}.[{{attributemapping.AttributesSource.Code}}]
{% elif CodeModel|upper != 'DA_CENTRAL' and Expression not in attributemapping %}[{{attributemapping.AttributeTarget.Code}}]  = {{attributemapping.AttributesSource.EntityAlias}}.[{{attributemapping.AttributesSource.Name}}]
{% endif %}
{%- if not loop.last -%}, {% endif %}
{% endfor %}
{% for sourceObject in mapping.SourceComposition %}
{% if sourceObject.JoinType != 'APPLY' %}
    {{ sourceObject.JoinType}} {{sourceObject.Entity.CodeModel}}.{{sourceObject.Entity.Name}} AS {{sourceObject.JoinAlias}} 
...
GROUP BY
{% for attributemapping in mapping.AttributeMapping %}
{% if 'Expression' not in attributemapping and not loop.first%},{{attributemapping.AttributesSource.EntityAlias}}.[{{attributemapping.AttributesSource.Name}}]
{% elif 'Expression' not in attributemapping%}{{attributemapping.AttributesSource.EntityAlias}}.[{{attributemapping.AttributesSource.Name}}]
{% endif %}
{% endfor %}
;
